...
Where PNROutputGroupName = 'Canada Migration General Group')
		SET @PNROutputGroupIDInvoice =	(SELECT PNROutputGroupID
FROM [PNROutputGroup]
Where PNROutputGroupName = 'Canada Migration BackOffice Group')
		SET @PNROutputRemarkGroupId = (SELECT PNROutputRemarkGroupId
FROM PNROutputRemarkGroup
where PNROutputRemarkGroupName = 'CanadaAmadeusTktRemarks')
...
VALUES
	(@PNROutputItemId + 1, 0, 'S', 1, 'T', 'TKT%TktRemarkNbr%-VN-%VnCode%/BA-%PenaltyAmt%/TX1-%PenaltyGst%XG/TX2-%PenaltyHst%RC/TX3-%PenaltyQst%XQ/TX4-%OtherTax%XT', @CreationTimestamp, @CreationUserIdentifier, 1, 'en-GB', NULL),
...
VALUES
	(@PNROutputItemId + 5, 'fr-CA', 'LES FRAIS DE BILLET D AVION DE CET ITINERAIRE/FACTURE', @CreationTimestamp, @CreationUserIdentifier, 1),
...
VALUES
	(@PNROutputItemId + 2, 'IsNuc', 'true', @CreationTimestamp, @CreationUserIdentifier, 1),
