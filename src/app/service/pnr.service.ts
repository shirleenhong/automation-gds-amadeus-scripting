...
export class PnrService {
        ...
        if (pass.length > 0) {
            pass.forEach((x) => {
                ...
                if (!found && list.length === x.segments.length) {
                    ...
                    const air = this.getSegmentList().find((z) => z.segmentType === 'AIR' && z.controlNumber === model.supplierConfirmatioNo);
                    if (air) {
    ...
    getTicketList() {
        ...
        for (const ticketed of this.pnrObj.faElements) {
            ...
            const tkt = ticketed.freeFlowText.split('/');
            ticketList.push({
                freeFlowText: `${tkt[0]} / ${tkt[1]} / ${tkt[2]} / ${tkt[5]}`,
    ...
    getRME() {
        ...
        if (this.isPNRLoaded) {
            const rmList = this.pnrObj.rmElements;
