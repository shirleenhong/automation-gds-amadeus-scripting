...
leisureFeeList: Array<LeisureFeeModel> = [];
modalRef: BsModalRef;
isAddNew = false;
  ...
  this.modalService.onHide.subscribe(result => {
    if (this.modalRef !== undefined && this.modalRef.content !== undefined) {
      if (this.modalRef.content.isSubmitted) {
        if (!this.isAddNew) {
          const cur = this.leisureFeeList.find(x => x.num === this.modalRef.content.leisureFee.num);
...
addLeisureFee() {
  ...
  leisureFee.num = (this.leisureFeeList.length + 1);
