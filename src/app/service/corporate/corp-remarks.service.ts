...
export class CorpRemarksService {
    ...
    public getIrdInformation(): Array<IrdModel> {
        ...
        for (const rm of this.pms.pnrObj.rmElements) {
            let regex = /-+CWT IRD RATE [NBR|NMR]{3}\s(?<nbrNo>[a-zA-Z0-9]+)-+(?<irdStatus>.*)/g;
            ...
            regex = /-+LOW FARE OPTION SAVINGS ACHIEVED\s(?<lowFareSavings>[0-9\.]+)-+/g;
            match = regex.exec(rm.freeFlowText);
            if (match) {
    ...
    public writeIrdRemarks(ird: IrdRemarksComponent, isUSOID: boolean): void {
        ...
        if (items.length > 0) {
            this.queService.addQueueCollection(new QueuePlaceModel(
                isUSOID ? 'MSPWL24GC' : 'YTOWL210N',
                isUSOID ? 49 : 40,
                isUSOID ? 49 : 285));
        }
    }
